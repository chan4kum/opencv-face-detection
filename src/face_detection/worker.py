...
Run with ``uvicorn face_detection.worker:create_app --factory``. The process is a small FastAPI
...
from face_detection.api.app import install_error_handlers, load_detector
...
from face_detection.jobs import JobBus, JobMessage, JobRecord, JobStatus
from face_detection.logging_config import configure_logging, get_logger
from face_detection.metrics import BUILD_INFO, JOB_E2E_DURATION, JOBS_PROCESSED, WORKER_IN_FLIGHT
from face_detection.schemas import HealthResponse
from face_detection.service import InferenceService
from face_detection.storage import ObjectNotFoundError, ObjectStore
from face_detection.telemetry import get_tracer, setup_tracing
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        await bus.connect()
        worker = Worker(settings, bus, store, InferenceService(detector, settings))
        worker.start()
