...
from face_detection.annotate import draw_faces, encode_jpeg
from face_detection.api.security import CurrentPrincipal
from face_detection.api.state import AppState, get_state
from face_detection.errors import (
    AsyncDisabledError,
    NotFoundError,
    PayloadTooLargeError,
    UnsupportedMediaTypeError,
)
from face_detection.imaging import probe_image
from face_detection.jobs import JobMessage, JobStatus, is_valid_job_id, new_job_id
from face_detection.metrics import IMAGES_REJECTED, JOBS_SUBMITTED
from face_detection.schemas import (
    DetectResponse,
    HealthResponse,
    JobAccepted,
    JobResponse,
...
async def model_info(state: State, _: CurrentPrincipal) -> ModelInfo:
    return state.inference.model


@v1.post(
    "/detect",
    response_model=DetectResponse,
    summary="Detect faces (synchronous)",
    description="Send the raw image bytes as the request body with an `image/*` Content-Type.",
)
async def detect(request: Request, state: State, _: CurrentPrincipal) -> DetectResponse:
    data = await read_image_body(request, state.settings.max_upload_bytes)
    result, _image, _faces = await state.inference.detect_bytes(data, source="api")
    return DetectResponse(request_id=request.state.request_id, **result.model_dump())
...
@v1.post(
    ...
    summary="Detect faces and return the annotated image",
)
async def detect_annotated(request: Request, state: State, _: CurrentPrincipal) -> Response:
    data = await read_image_body(request, state.settings.max_upload_bytes)
    result, image, faces = await state.inference.detect_bytes(data, source="api")
