...
{{- define "fd.env" -}}
- {name: FD_ENVIRONMENT, value: prod}
- {name: FD_LOG_JSON, value: "true"}
- {name: FD_MODEL_PATH, value: /app/models/face_detection_yunet_2026may.onnx}
...
- {name: FD_AUTH_DISABLED, value: "true"}
{{- else }}
- name: FD_API_KEY_HASHES
  valueFrom:
    secretKeyRef:
...
- {name: FD_ASYNC_ENABLED, value: "true"}
- {name: FD_NATS_URL, value: {{ include "fd.natsUrl" . | quote }}}
- {name: FD_NATS_REPLICAS, value: {{ .Values.async.replicas | quote }}}
- {name: FD_NATS_STREAM, value: {{ .Values.async.stream | quote }}}
- {name: FD_NATS_CONSUMER, value: {{ .Values.async.consumer | quote }}}
- {name: FD_S3_BUCKET, value: {{ .Values.storage.bucket | quote }}}
- {name: FD_S3_REGION, value: {{ .Values.storage.region | quote }}}
- {name: FD_S3_FORCE_PATH_STYLE, value: {{ .Values.storage.forcePathStyle | quote }}}
{{- if .Values.storage.endpointUrl }}
- {name: FD_S3_ENDPOINT_URL, value: {{ .Values.storage.endpointUrl | quote }}}
{{- end }}
{{- if .Values.storage.serverSideEncryption }}
- {name: FD_S3_SERVER_SIDE_ENCRYPTION, value: {{ .Values.storage.serverSideEncryption | quote }}}
{{- end }}
{{- if .Values.storage.existingSecret }}
- name: FD_S3_ACCESS_KEY_ID
  valueFrom: {secretKeyRef: {name: {{ .Values.storage.existingSecret }}, key: access-key-id}}
- name: FD_S3_SECRET_ACCESS_KEY
