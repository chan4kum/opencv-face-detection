...
Enabled by exporting:
    FD_TEST_NATS_URL=nats://127.0.0.1:4222
    FD_TEST_S3_ENDPOINT=http://127.0.0.1:8333
    FD_TEST_S3_ACCESS_KEY=...  FD_TEST_S3_SECRET_KEY=...
...
from face_detection.api.app import create_app, load_detector
from face_detection.config import Settings, hash_api_key
from face_detection.jobs import JobBus, JobMessage
from face_detection.service import InferenceService
from face_detection.storage import ObjectStore
from face_detection.worker import Worker
from tests.conftest import make_settings

NATS_URL = os.environ.get("FD_TEST_NATS_URL")
S3_ENDPOINT = os.environ.get("FD_TEST_S3_ENDPOINT")

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skipif(not (NATS_URL and S3_ENDPOINT), reason="FD_TEST_NATS_URL / FD_TEST_S3_ENDPOINT not set"),
]
...
async def rig() -> AsyncIterator[Rig]:
    ...
    settings = make_settings(
        ...
        s3_access_key_id=os.environ.get("FD_TEST_S3_ACCESS_KEY", "devaccesskey"),
        s3_secret_access_key=os.environ.get("FD_TEST_S3_SECRET_KEY", "devsecretkey-change-me"),
    ...
    async def start_worker() -> Worker:
        ...
        await bus.connect()
        worker = Worker(settings, bus, wstore, InferenceService(load_detector(settings), settings))
        worker.start()
...
async def test_worker_app_serves_probes_and_metrics(rig: Rig) -> None:
    ...
    from face_detection.worker import create_app as create_worker_app
    ...
    async with (
        ...
        assert "fd_worker_jobs_in_flight" in (await c.get("/metrics")).text
