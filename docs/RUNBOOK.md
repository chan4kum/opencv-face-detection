...
Each alert in `deploy/prometheus/alerts.yml` links to a section here. Metric names are `fd_*` (see ARCHITECTURE.md).

## Quick triage

```bash
kubectl get pods -l app.kubernetes.io/name=face-detection
kubectl logs deploy/<release>-face-detection-api --tail=100 | jq -c 'select(.level!="info")'
kubectl logs deploy/<release>-face-detection-worker --tail=100 | jq -c .
...
## High error rate
*Alert `FaceDetectionHighErrorRate`: more than 2% of requests are 5xx for 10 min.*

1. Split by status/route: `sum by (route,status) (rate(fd_http_requests_total{status=~"5.."}[5m]))`.
...
## High latency
*Alert `FaceDetectionHighLatency`: p95 of `/v1/detect` above 500 ms for 10 min.*

1. Compare `fd_inference_duration_seconds` to `fd_http_request_duration_seconds`. If inference is fast but requests slow, the time is
...
## Load shedding
*Alert `FaceDetectionLoadShedding`: requests are answered 503 `overloaded` (`fd_inference_rejected_total`).*
...
## Jobs failing
*Alert `FaceDetectionJobsFailing`: `fd_jobs_processed_total{outcome="failed_exhausted"|"poison"}` is increasing.*
...
2. Object storage reachable? `kubectl exec` a worker and run `python -c "from face_detection.storage import ObjectStore; ..."`, or check `/readyz` on the API.
3. NATS KV writable? `nats kv status fd_jobs` (nats-box).
4. `poison` messages: something other than this API published to the subject. Inspect with `nats stream view FD_JOBS`.

## Queue backlog
*Alert `FaceDetectionQueueBacklog`: more than 500 pending messages for 10 min.*
...
3. `nats consumer info FD_JOBS fd-workers`: high `Ack Pending` with no progress means stuck jobs; they are redelivered after `job_ack_wait_s`.
...
| Task | How |
|---|---|
| Rotate an API key | `face-detection keygen`; add the new digest to `auth.apiKeyHashes`, roll clients, remove the old digest |
...
| Verify a model file | `face-detection verify-model --model path.onnx` |
