# ADR 0002: NATS JetStream for queueing and job state, S3 API for image payloads
...
- (+) Storage backend is swappable through configuration only (`FD_S3_ENDPOINT_URL`).
