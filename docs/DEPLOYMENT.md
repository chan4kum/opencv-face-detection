...
| Grafana (dashboard "Face Detection Service") | http://localhost:3000 |
...
Auth is off by default locally; set `FD_API_KEY_HASHES` to enable it.
...
uv run face-detection keygen

helm dependency build deploy/helm/face-detection
helm install fd deploy/helm/face-detection \
  --namespace face-detection --create-namespace \
  --set auth.apiKeyHashes=<digest> \
  --set storage.endpointUrl=<s3-endpoint> --set storage.existingSecret=<secret> \
  --set api.ingress.enabled=true --set api.ingress.hosts[0].host=faces.example.com
helm test fd -n face-detection
```

Published chart and image (after a release tag): `oci://ghcr.io/chan4kum/charts/face-detection`,
`ghcr.io/chan4kum/opencv-face-detection`. Verify the signature before deploying:

```bash
cosign verify ghcr.io/chan4kum/opencv-face-detection@sha256:<digest> \
  --certificate-identity-regexp 'https://github.com/chan4kum/opencv-face-detection/.*' \
...
Minimum S3 IAM policy for the workload role (bucket `face-detection`):

```json
{
  "Version": "2012-10-17",
  "Statement": [
    {"Effect": "Allow", "Action": ["s3:PutObject", "s3:GetObject", "s3:DeleteObject"], "Resource": "arn:aws:s3:::face-detection/inputs/*"},
    {"Effect": "Allow", "Action": ["s3:ListBucket"], "Resource": "arn:aws:s3:::face-detection"}
  ]
}
```
...
* JetStream stream/KV settings are reconciled on start (`FD_NATS_PROVISION=true`). Set it to `false` and manage them yourself if you prefer.
